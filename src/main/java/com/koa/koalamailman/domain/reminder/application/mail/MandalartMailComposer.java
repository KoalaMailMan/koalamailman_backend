...
import com.koa.koalamailman.domain.reminder.domain.MandalartReminder;
...
public class MandalartMailComposer {
    ...
    public MailMessage compose(MandalartReminder reminder) {

        String html = buildHtml(reminder);

        return MailMessage.builder()
                .from(from)
                .to(reminder.getEmail())
                .subject(subject)
                .html(html)
                .build();
    }

    private String buildHtml(MandalartReminder reminder) {

        String name = esc(reminder.getUsername());
        String title = name.isBlank()
                ? "코알라 우체부에게서, 편지가 왔어요  🐨🪽✉️"
                : name + "님, 코알라 우체부가 편지를 전해요  🐨🪽✉️";
        String[][] grid = reminder.getGrid().values();
        ...
        return """
                ...
                .formatted(
                                ...
                                inner + font,
                                panel + font,
                                "padding:16px 18px;" + font, reminder.getTip(),
                                inner + font,
                                ctaUrl, ctaStyle,
                                inner + font
                        );
